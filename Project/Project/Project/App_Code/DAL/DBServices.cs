...
private string BuildInsertCommand_Stone(Stone S)
{
    ...
    // Read from the DB into a table
    //--------------------------------------------------------------------
    public DBServices ReadFromDataBase_Filter(string conString, string selectStr)
    {
        DBServices dbS = new DBServices(); // create a helper class
        SqlConnection con = null;
    ...
    // Read from the DB into a table
    //--------------------------------------------------------------------
    public DBServices ReadFromDbView(string conString, string viewName, string id)
    {
        DBServices dbS = new DBServices(); // create a helper class
        SqlConnection con = null;
        ...
        {
            con = dbS.connect(conString); // open the connection to the database/
            String selectStr = "SELECT * FROM " + viewName + " WHERE Stone_Name = '" + id + "'"; // create the select that will be used by the adapter to select data from the DB
            SqlDataAdapter da = new SqlDataAdapter(selectStr, con); // create the data adapter
            DataSet ds = new DataSet(); // create a DataSet and give it a name (not mandatory) as defualt it will be the same name as the DB
            da.Fill(ds);                        // Fill the datatable (in the dataset), using the Select command
            DataTable dt = ds.Tables[0];
